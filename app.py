...
# （可选）若仍需端口，保留但建议动态化：
# app.config['SERVER_NAME'] = '0.0.0.0:1126'  # 需注释/删除以适配Render
...
# 哲学社信件（完全保留）
philosophy_letter = """福清一中“凌空”哲学社给新生们的一封信...（原文完整保留）"""
...
# ---------------------- 启动入口（移除SocketIO，改用纯Flask） ---------------------- #
if __name__ == '__main__':
    port = int(os.environ.get("PORT", 1126))  # 适配Render动态端口
    app.run(
        host='0.0.0.0',
        port=port,
        debug=False,
